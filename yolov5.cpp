...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw, std::string& img_dir, std::string& video_dir, std::string& rtsp) {
    ...
    if (std::string(argv[1]) == "-s" && (argc == 5 || argc == 7)) {
        ...
        engine = std::string(argv[2]);
	video_dir = std::string(argv[3]);
    } else if (std::string(argv[2]) == "-r" && argc == 4) {
        engine = std::string(argv[2]);
	rtsp = std::string(argv[3]);
    } else {
        return false;
    }
    return true;
...
int main(int argc, char** argv) {
    ...
    std::string rtsp;
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir, video_dir, rtsp)) {
	...
	std::cerr << "./yolov5 -v [.engine] ../videos   // deserialize plan file and run inference" << std::endl;
	std::cerr << "./yolov5 -r [.engine] rtsp // deserialize plan file and run inference" << std::endl;
        return -1;
    }
	...
	//std::cout << fcount << std::endl;
	    ...
	    //std::cout << "sss" << "_" + file_names[f - fcount + 1 + b] << std::endl;
	    //for(int i = 0; i < 100000; i++) cv::imwrite("_" + file_names[f - fcount + 1 + b], img);
